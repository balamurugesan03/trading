...
// Total investment volume generated by a user's own direct referrals (not their whole
// downline) - used to gate level income eligibility below.
...
// Each upline is also checked independently against levelIncomeQualificationBusiness - an
// upline whose own direct referrals haven't reached that combined investment volume yet is
// skipped entirely for this tier, regardless of the levels above/below them.
async function distributeLevelIncome(investment, roiAmount) {
  ...
  for (let i = 0; i < levelCount; i += 1) {
    ...
    const directBusiness = await getDirectBusiness(uplineId);
    if (directBusiness < settings.levelIncomeQualificationBusiness) continue; // eslint-disable-line no-continue
